...
export default function SkillsShowcase() {
  ...
  return (
    <div className="bg-white border border-slate-200 p-6 rounded-lg hover:border-cyan-500 transition-colors">
      <h2 className="text-xl font-bold text-slate-900 mb-2">Technical Skills</h2>
      <p className="text-sm text-slate-500 mb-6">My tech stack and proficiency levels</p>
          ...
          <button
            ...
            className={`p-3 lg:p-4 rounded-lg text-left transition-all ${
              activeCategory === cat.id
                ...
                : 'bg-slate-100/30 border-2 border-slate-600/30 hover:bg-slate-700/50 hover:border-slate-600/50'
            }`}
          >
            <div className="text-xl lg:text-2xl mb-1 lg:mb-2">{cat.icon}</div>
            <div className={`font-semibold text-sm mb-1 ${
              activeCategory === cat.id ? 'text-cyan-400' : 'text-slate-800'
            }`}>
              {cat.label}
            </div>
        ...
        {skills[activeCategory].map((skill, index) => (
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-slate-700 group-hover:text-cyan-400 transition-colors">
                {skill.name}
              </span>
              <span className="text-xs text-slate-900 font-mono bg-slate-700/50 px-2 py-0.5 rounded">
                {skill.level}%
              </span>
            </div>
            <div className="w-full bg-slate-400/50 rounded-full h-2 overflow-hidden">
              <div 
